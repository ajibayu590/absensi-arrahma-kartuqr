...
| 2026-07-08 | .gitignore Security | Menambah aturan `.gitignore` untuk keamanan publikasi ke GitHub dan mengatur kontrol versi. | Menambahkan entri untuk `/dist`, `logs/`, `*.log`, `/prisma/generated`, `/prisma/migrations`, `**/.prisma/`, `/tmp`, dan spesifikasi `.env*` yang lebih ketat. | Selesai |
| 2026-07-08 | PWA Basic Implementation | Implementasi dasar PWA untuk Android dan iOS. | Membuat `public/service-worker.js` dan mendaftarkannya di `src/app/layout.tsx`. Menambahkan `beforeinstallprompt` dan `appinstalled` event listener. | Selesai |
...
| 2026-07-08 | Endpoint Security Audit: /api/attendance/live-stream & /api/token-qr | Mengaudit keamanan endpoint `/api/attendance/live-stream` (SSE) dan `/api/token-qr` (QR token generation) terhadap akses tidak sah. | Ditemukan bahwa kedua endpoint tidak memiliki otentikasi, memungkinkan akses publik ke data sensitif dan pembuatan token. Diperbaiki dengan menambahkan otentikasi JWT dan otorisasi berbasis peran untuk akses terbatas. | Selesai |
