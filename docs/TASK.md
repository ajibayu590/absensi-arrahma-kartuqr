...
| 2026-07-08 | Endpoint Security Audit: /api/attendance/auto-alpha | Mengaudit keamanan endpoint `/api/attendance/auto-alpha` terhadap akses tidak sah. | Ditemukan bahwa endpoint sudah dilindungi dengan header `X-Scheduler-Secret` untuk scheduler dan otorisasi peran `ADMIN` untuk pemicu manual, mencegah spam WA. | Selesai |
