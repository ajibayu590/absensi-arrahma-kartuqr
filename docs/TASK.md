...
| 2026-07-08 | Centralized Authentication (middleware.ts) | Mengimplementasikan middleware otentikasi terpusat untuk Next.js. | Membuat `src/middleware.ts` untuk memeriksa token JWT dan melampirkan payload pengguna ke header `x-user-payload` sebelum permintaan mencapai route. Semua API routes yang dilindungi dimodifikasi untuk membaca payload dari header. | Selesai |
